...
def main():
    ...
    parser.add_argument('--input_model_file', type=str, default='',
                        help='filename to input the pre-trained model')
    ...
    model_substruct.load_state_dict(torch.load(args.input_model_file + ".pth", map_location=lambda storage, loc: storage))

    model_context = GNN(3, args.emb_dim, JK = args.JK, drop_ratio = args.dropout_ratio, gnn_type = args.gnn_type).to(device)

    #model_context.load_state_dict(torch.load(args.input_model_file + ".pth", map_location=lambda storage, loc: storage))
