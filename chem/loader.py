...
def mol_to_graph_data_obj_simple(mol):
    ...
    nodes_num = len(atom_features_list)
    ...
    if len(mol.GetBonds()) > 0:  # mol has bonds
        ...
        graph_adj = torch.sparse_coo_tensor(indices=edge_index.clone().detach(),
                                            values=torch.ones((len(edges_list)), dtype=torch.int),
                                            size=(nodes_num, nodes_num))
                                          

    else:  # mol has no bonds
        edge_index = torch.empty((2, 0), dtype=torch.long)
        edge_attr = torch.empty((0, num_bond_features), dtype=torch.long)
        # graph_adj = edge_index.clone().detach().t()
        graph_adj = torch.sparse_coo_tensor(indices=edge_index.clone().detach().t(),
                                            values=torch.ones((len(edge_index)), dtype=torch.int),
                                            size=(len(mol.GetAtoms()), len(mol.GetAtoms())))

    data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, graph_adj=graph_adj)
    print(data)

    return data


def mol_to_graph_data_obj_adj(mol, cluster_number):
    """
    Converts rdkit mol object to graph Data object required by the pytorch
    geometric package. NB: Uses simplified atom and bond features, and represent
    as indices
    ...
    data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, graph_cluster_distance=pseudo_labels)
    # data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, graph_cluster_distance=pseudo_labels)
    # print(data)
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                # data = mol_to_graph_data_obj_simple(rdkit_mol)
                    data = mol_to_graph_data_obj_adj(rdkit_mol, self.cluster_number)
                    ...
                    num_success += 1
                    print('succeed:',i)
                    #print("total index: {}".format(i))
                    #print("success index: {}".format(num_success))
...
if __name__ == "__main__":
    ...
    for dataset_name in downstream_dir:
        ...
        root = "dataset/" + dataset_name
        # os.makedirs(root + "/processed", exist_ok=True)
        dataset = MoleculeDataset(root, dataset=dataset_name, empty=True)
        dataset.process()
